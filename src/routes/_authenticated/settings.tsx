...
function SettingsPage() {
  ...
  const { data: settings } = useQuery({
    ...
    queryFn: async () => {
      const { data: u } = await supabase.auth.getUser();
      if (!u.user) return null;
      const { data } = await supabase.from("user_settings").select("*").eq("user_id", u.user.id).single();
      return data;
    },
  });
  ...
  const addRule = useMutation({
    mutationFn: async () => {
      ...
      if ((rules?.length ?? 0) >= MAX_CUSTOM_RULES) throw new Error(`Max ${MAX_CUSTOM_RULES} rules`);
      const { data: u } = await supabase.auth.getUser();
      if (!u.user) return;
  ...
  return (
    <div className="min-h-screen pb-24">
      <BrandHeader right={<button onClick={signOut} className="flex items-center gap-1 rounded-full border border-border px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider text-muted-foreground"><LogOut className="h-3 w-3" />Sign out</button>} />
      <main className="mx-auto max-w-3xl px-4 pt-6 space-y-5">
        ...
        <div className="rounded-3xl bg-card p-4 glow-blue-soft">
          ...
          <div className="space-y-2">
            <F label="Name"><input value={displayName} onChange={(e) => setName(e.target.value)} className={inp} /></F>
            <F label="Account Balance ($)"><input type="number" step="0.01" value={displayBalance} onChange={(e) => setBalance(e.target.value)} className={inp} /></F>
            <F label="USD → INR rate"><input type="number" step="0.01" value={displayRate} onChange={(e) => setRate(e.target.value)} className={inp} /></F>
            <F label="Default currency display">
        ...
        <div className="rounded-3xl bg-card p-4 glow-blue-soft">
          <div className="mb-3 flex items-center justify-between">
            ...
            <span className="text-[10px] text-muted-foreground">{rules?.length ?? 0} / {MAX_CUSTOM_RULES}</span>
          </div>
          <div className="space-y-1.5">
            {(rules ?? []).map((r, i) => (
              <div key={r.id} className="flex items-center gap-2 rounded-xl border border-border bg-input px-3 py-2.5">
                ...
                <button onClick={() => delRule.mutate(r.id)} className="text-muted-foreground hover:text-destructive"><X className="h-4 w-4" /></button>
              </div>
            ))}
          </div>
          {(rules?.length ?? 0) < MAX_CUSTOM_RULES && (
            <div className="mt-3 flex gap-2">
              <input
                value={newRule}
                onChange={(e) => setNewRule(e.target.value)}
                placeholder="Add a rule (one-liner)"
                className="flex-1 rounded-xl border border-border bg-input px-3 py-2.5 text-sm outline-none focus:border-primary"
              />
              <button
                onClick={() => addRule.mutate()}
                className="flex items-center gap-1 rounded-xl bg-primary px-4 text-sm font-bold text-primary-foreground glow-blue"
